# k0s Coder Template Module
# Single-node Kubernetes cluster for development and CI/CD testing
# Uses Kubernetes with sysbox-runc (k0s runs inside the pod)
...
variable "k0s_version" {
  type        = string
  description = "k0s version to use (default: installed in image)"
  default     = ""
}
...
data "coder_parameter" "disk_size" {
  ...
  description  = "Storage for home directory, Docker images, and Kubernetes data"
  default      = "100"
  type         = "number"
  mutable      = false
  validation {
...
resource "coder_agent" "main" {
  ...
  env = {
    ...
    CODER_MCP_CLAUDE_SYSTEM_PROMPT = "You are a helpful AI assistant for the ${var.project_name} development workspace with Kubernetes (k0s) support."
  ...
  shutdown_script = <<-EOT
    #!/bin/bash
    echo "=== Shutdown ==="
    /opt/coder-scripts/stop-k0s.sh || true
  ...
  startup_script = <<-EOT
    #!/bin/bash
    set -e
    echo "=== Starting ${var.project_name} workspace with k0s ==="
    ...
    /opt/coder-scripts/setup-k0s.sh
    ...
    # Start docker-compose services if compose file exists
...
module "workspace" {
  ...
  image             = "ghcr.io/codespacesh/k0s:latest"
...
resource "coder_app" "k9s" {
  agent_id     = coder_agent.main.id
  slug         = "k9s"
  display_name = "k9s (Kubernetes TUI)"
  command      = "k9s"
  icon         = "/icon/k8s.png"
  share        = "authenticated"
}
...
resource "coder_metadata" "workspace" {
  ...
  item {
    key   = "Kubernetes"
    value = "k0s single-node with metrics-server, Traefik, local-path"
  }
}
